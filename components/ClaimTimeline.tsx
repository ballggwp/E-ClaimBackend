...
export function ClaimTimelineWithDates({ statusDates, currentStatus }: Props) {
  ...
  return (
      ...
      >
        {/* background track */}
        <div className="absolute inset-y-0 flex items-center z-0" style={{ left: `${50/total}%`, right: `${50/total}%` }}>
          <div className="w-full h-1 bg-gray-200 rounded-full" />
        </div>

        {/* filled track */}
        <div className="absolute inset-y-0 flex items-center z-10 pointer-events-none" style={{ left: `${50/total}%`, right: `${50/total}%` }}>
          <div
        ...
        {STEPS.map((step, i) => {
          ...
          const dotColor = isDone ? "bg-green-500" : isActive ? "bg-yellow-400" : "bg-gray-300";
          const textColor = isDone || isActive ? "text-white" : "text-gray-500";

          const iso = i < total - 1 ? statusDates[STEPS[i + 1].code] : statusDates[step.code];
          const tsLabel = iso
            ? format(new Date(iso), "dd/MM/yy HH:mm", { locale: th })
            : "–";

          return (
            <div key={step.code} className="relative flex flex-col items-center z-20">
              <span className="mb-2 text-xs text-gray-600">{tsLabel}</span>
              <div
                className={
                  `w-10 h-10 rounded-full ${dotColor} ring-4 ring-white shadow-md flex items-center justify-center ${
